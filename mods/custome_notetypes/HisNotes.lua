function onCreate()
	--Iterate over all notes
	for i = 0, getProperty('unspawnNotes.length')-1 do
		--Check if the note is an Instakill Note
		if getPropertyFromGroup('unspawnNotes', i, 'noteType') == 'HisNotes' then
			setPropertyFromGroup('unspawnNotes', i, 'texture', 'HisNotes'); --Change texture
			setPropertyFromGroup('unspawnNotes', i, 'missHealth', '0'); --Default value is: health lost on miss
			setPropertyFromGroup('unspawnNotes', i, 'hitCausesMiss', false);
			setPropertyFromGroup('unspawnNotes', i, 'lowPriority', true) --lowPriority means it has less of a chance to be hit.

			if getPropertyFromGroup('unspawnNotes', i, 'mustPress') then --Doesn't let Dad/Opponent notes get ignored
				setPropertyFromGroup('unspawnNotes', i, 'ignoreNote', true); --Miss has no penalties
			end
		end
	end
	--debugPrint('Script started!')
end

-- Function called when you hit a note (after note hit calculations)
-- id: The note member id, you can get whatever variable you want from this note, example: "getPropertyFromGroup('notes', id, 'strumTime')"
-- noteData: 0 = Left, 1 = Down, 2 = Up, 3 = Right
-- noteType: The note type string/tag
-- isSustainNote: If it's a hold note, can be either true or false
function goodNoteHit(id, noteData, noteType, isSustainNote)
	if noteType == 'HisNotes' then
		health = getProperty('health')
		setProperty('health', getProperty('health')-0.25);
		triggerEvent('Play Animation', 'firstDeath', 'bf')
		makeAnimatedLuaSprite('hisfx', 'bg/vintage', 0, 0)
		addAnimationByPrefix('hisfx', 'idle', 'idle', 16, true)
		scaleObject('hisfx', 3, 3)
		setObjectCamera('hisfx', 'other')
		objectPlayAnimation('hisfx', 'idle', true)
		doTweenAlpha('hehefxfade', 'hisfx', 0, 1, 'linear')
		addLuaSprite('hisfx', true)
		playSound('hitStatic', 1);
	end
end